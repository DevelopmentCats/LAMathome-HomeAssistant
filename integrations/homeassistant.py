...
async def homeassistant(user_input):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(f"{HA_URL}/api/states", headers=headers) as response:
            if response.status != 200:
                return f"Failed to retrieve states: {response.status}"
        ...
        async with session.post(url, json=payload, headers=headers) as response:
            if response.status == 200:
                ...
                return f"Failed to control {best_match}: {response.status}"

# Example usage
async def main():
    result = await homeassistant("Turn on living room lights")
    print(result)
